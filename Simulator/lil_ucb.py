...
def lil_ucb(students, arms, delta, epsilon, lambda_p, beta, sigma):
    # delta == confidence
    # 
    time = 0
    n = len(arms)
    ...
    for i in range(n):
        T[i] = 1
        mu[i] = sim.simulate(armList[i].get_arm(), students) #pull the arm
    ...
    while True:
        ...
        T[index] += 1
        reward = sim.simulate(armList[index].get_arm(), students)
